...
not the current queues. Message-list preference, saved-search, signature, and
mailbox-color owner records also move as their existing authenticated ciphertext;
Redis, not their archived local files, is current after migration. Saved-search
and signature revisions plus mailbox-color read-modify-write operations remain
