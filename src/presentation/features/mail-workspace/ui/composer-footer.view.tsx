...
export const ComposerFooterView = ({
  ...
  const draftMutationPending = composer.draft.phase === "saving";
  const announcement = [
    composer.draft.status?.announcement,
  ...
  <>
    ...
    <div className="flex min-h-15 shrink-0 flex-wrap items-center gap-1 border-t border-slate-100 px-3 py-2">
      <button
        ...
        disabled={composer.isBusy || draftMutationPending || !composer.draft.canSend || composer.isUploading || composer.attachments.some(({ state }) => state === "error")}
        type="submit"
      >
      ...
      <label
        ...
        className={`grid size-9 place-items-center rounded-xl focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-indigo-600 ${
          composer.maxAttachmentBytes > 0 && !composer.isBusy &&
            composer.draft.canAttach && composer.draft.canEdit
      ...
      >
        ...
        <input
          className="sr-only"
          disabled={composer.isBusy || !composer.draft.canAttach ||
            !composer.draft.canEdit || composer.maxAttachmentBytes <= 0}
      ...
      {composer.draft.enabled ? (
        <>
          <button
            ...
            disabled={composer.isBusy || draftMutationPending || !composer.draft.canSave}
            onClick={composer.draft.onSave}
            type="button"
          >
            <Save aria-hidden size={15} /> Save draft
          </button>
          {(!composer.draft.terminalRecovery &&
            (composer.draft.requiresRecovery ||
            (composer.draft.phase === "error" &&
              (composer.draft.canSave || composer.draft.loadFailed)))) ? (
            <button className="rounded-lg px-2 py-1 text-xs font-bold text-indigo-700 hover:bg-indigo-50 disabled:text-slate-400" disabled={composer.isBusy || draftMutationPending} onClick={composer.draft.onRetry} type="button">
              {composer.draft.requiresRecovery
                ? "Recover saved draft"
          ...
          {composer.draft.phase === "conflict" && !composer.draft.requiresRecovery && composer.draft.onReload ? (
            <button className="rounded-lg px-2 py-1 text-xs font-bold text-indigo-700 hover:bg-indigo-50 disabled:text-slate-400" disabled={composer.isBusy || draftMutationPending} onClick={composer.draft.onReload} type="button">
              Reload saved draft
            </button>
          ) : null}
        </>
      ...
      {composer.draft.status ? (
        <span className={`text-xs font-semibold ${statusTone[composer.draft.status.tone]}`}>
          {composer.draft.status.label}
        </span>
      ) : null}
      {composer.draft.requiresRecovery &&
        composer.draft.terminalRecovery === "send" ? (
        <button className="rounded-lg px-2 py-1 text-xs font-bold text-indigo-700 hover:bg-indigo-50 disabled:text-slate-400" disabled={composer.isBusy || draftMutationPending} onClick={composer.draft.onRetry} type="button">
          I checked Sent — resume as draft
        </button>
      ) : null}
      ...
      {composer.draft.sendBlockedMessage ? (
        <span className="basis-full text-xs font-semibold text-amber-800">
          {composer.draft.sendBlockedMessage}
        </span>
      ) : null}
      ...
      <button
        ...
        disabled={composer.isBusy || draftMutationPending || !composer.draft.canDiscard}
        id="composer-discard"
