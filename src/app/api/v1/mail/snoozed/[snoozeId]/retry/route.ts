...
import { retrySnooze } from "@/server/snooze/snooze-service";
...
export const POST = async (request: Request, context: RouteContext) => {
  try {
    ...
    return apiSuccess(await retrySnooze(connection, snoozeId));
  } catch (error) { return apiFailure(error, "Unable to retry this snooze."); }
};
