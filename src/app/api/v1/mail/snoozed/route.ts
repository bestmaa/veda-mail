...
export const GET = async (request: Request) => {
  try {
    await assertRequestRateLimit(request, "snooze-read", 10_000, 300, 60_000);
    const connection = await getCurrentConnection();
    assertMailSessionScope(request, connection);
    await assertSubjectRateLimit("snooze-read", connection.id, 120, 60_000);
...
export const POST = async (request: Request) => {
  try {
    ...
    await assertSubjectRateLimit("snooze-write", connection.id, 100, 15 * 60_000, items.length);
    assertMailSessionScope(request, connection);
