...
const remove = (connectionId: ConnectionId, expectedCreatedAt?: string): boolean => {
  ...
  if (expectedCreatedAt && stored.connection.createdAt !== expectedCreatedAt) return false;
...
const scheduleExpiry = (stored: StoredConnection): void => {
  ...
  if (remaining <= 0) {
    remove(connection.id, connection.createdAt);
    return;
  }
  const timer = setTimeout(() => remove(connection.id, connection.createdAt), remaining);
  timer.unref();
...
const prune = (): void => {
  for (const [connectionId, stored] of state.connections) {
    ...
    if (expiresAt === null || expiresAt <= Date.now()) {
      remove(connectionId, stored.connection.createdAt);
    }
  }
};

for (const stored of state.connections.values()) scheduleExpiry(stored);

export const localConnectionStore = {
  beginSendIfActive(
    ...
    const expiresAt = storedConnectionExpiresAt(stored);
    if (expiresAt === null || expiresAt <= Date.now()) {
      remove(connection.id, current.createdAt);
      return { kind: "inactive" };
    }
    return sendIdempotencyStore.begin(connection.id, draftId, fingerprint, expiresAt);
  },

  appendDeliveryNoticeIfActive(connection: ProviderConnection, receipt: SendReceipt): boolean {
    prune();
    const stored = state.connections.get(connection.id);
    const current = stored?.connection;
    if (!stored || !current || current.createdAt !== connection.createdAt ||
    ...
    if (!admitted) state.connections.set(connection.id, {
      ...stored, deliveryNoticeCapacityWarning: true,
    });
    return admitted;
  },
  ...
  isActive(connection: ProviderConnection): boolean {
    ...
    return Boolean(current && current.createdAt === connection.createdAt &&
      current.providerId === connection.providerId);
  },

  remove(connectionId: ConnectionId): void { remove(connectionId); },
  ...
  updateConfig(
    ...
    prune();
    const stored = state.connections.get(connectionId);
    if (!stored) throw new Error("Mail connection was not found.");
